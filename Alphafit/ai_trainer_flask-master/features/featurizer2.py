...
def start_to_squat(squat,key,multiple=0.5):
    return squat.iloc[[get_midpoint(squat,start=True,key=key,multiple=multiple)]]
...
def squat_to_end(squat,key,multiple=0.5):
    return squat.iloc[[get_midpoint(squat,start=False,key=key,multiple=multiple)]]
...
def get_general_feature_vector(squats, key, multiples):
    ...
    for squat in squats:
        ...
        squat = get_states(squat,key,multiples)
        features = np.concatenate([get_raw_coords(squat),get_coord_pairs(squat),get_angles(squat)],axis=0)
...
#=====[ Gets raw coordinates as specified in the coordinate file ]=====
def get_raw_coords(states):
    
    coords = bare_coords.coords
    
    #=====[ Concatenates all the raw coordinates for all the states ]=====
    features = np.concatenate([np.asarray(state[coords])[0] for state in states],axis=0)
    
    return features


import bare_coord_pairs

#=====[ Returns difference between matching pairs of coordinates (left/right) ]=====
def get_coord_pairs(states):

    pairs = bare_coord_pairs.pairs
    
    features= []
    dims = ['X','Y','Z']
    
    #=====[ Iterate over each state, pair of coodinates, and relevant dimensions ]=====
    for state in states:
        for pair in pairs:
            for dim in dims:
                features.append(np.abs(float(state[pair+'Left'+dim] - state[pair+'Right'+dim])))
...
#=====[Returns angles in the body and ratios of all angles ]=====
def get_angles(states):
    
    angles = bare_angles.angles
    features = []
    dim_pairs = [('X','Y'),('Z','Y'),('X','Z')]
    
    #=====[Iterate over states ]=====
    for state in states:
        state_features = []
        
        #=====[Iterate over each plane in which we're taking an angle]=====
        for dim in dim_pairs:
            
            #=====[ Iterate over each angle (set of 3 joints) specified in angles ]=====
            for idx, angle in enumerate(angles):
                
                cur_angle = float(get_angle(state,angle[0],angle[1],angle[2],dim[0],dim[1]))
                state_features.append(cur_angle)
                features.append(cur_angle)
                
                #=====[ Take the ratio between each angle and every angle that has previously been calculated
                #for a particular state ]=====
                for idx2 in range(0,idx):
                    features.append(cur_angle/state_features[idx2])
            
    return np.asarray(features)   


def transform_gen_data(features, labels, toIgnore, predict=False):
    X = {}
    Y = {}
